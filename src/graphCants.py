...
def _loss_short(cost_type: str) -> str:
    return {"mse": "MSE", "bicross_entropy": "BCE", "cross_entropy": "CE"}.get(cost_type, "LOSS")
...
class Graph:
 ...
 # ================== Accuracy helpers (one-hot aware) ==================
    def accuracy_one_hot(self, y_true, y_pred, prt=False):
        """
        Accepts either:
          - y_true as 0/1, y_pred as sigmoid probs over a single output
          - OR y_true one-hot [1,0]/[0,1], y_pred as softmax over 2 outputs
        Returns True if predicted class == true class.
        """
        ...
        # Try to infer shape
        if yp.ndim == 0:
            pred_class = int(yp >= 0.5)
        elif yp.ndim == 1:
            if yp.shape[0] == 1:
                pred_class = int(yp[0] >= 0.5)
            else:
                pred_class = int(np.argmax(yp))
        else:
            pred_class = int(np.argmax(yp))
        ...
        if yt.ndim == 0 or (yt.ndim == 1 and yt.shape[0] == 1):
            true_class = int(float(yt) >= 0.5)
        else:
            true_class = int(np.argmax(yt))

        ok = (pred_class == true_class)
        if prt:
            print(f"[ACC] pred_class={pred_class} true_class={true_class} ok={ok}")
        return ok

    def compute_accuracy(self, data, cost_type="cross_entropy", on="test", prt=False):
        """
        Compute accuracy (%) over a dataset split using one-hot targets if available.
        - data: Timeseries-like with .train_input/.train_output/.test_input/.test_output
        - cost_type controls activation inside single_thrust.
        - on: 'train' or 'test'
        """
    ...
    def evaluate(self, data, cal_gradient=True, cost_type="mse", num_epochs=1, thread_id=None):
        ...
        stage    = getattr(data, "_stage", "")
        interval = getattr(data, "_interval", "")
        tag = f"[{stage} | {interval}] " if (stage or interval) else ""
        thd = f" Thread({thread_id})" if thread_id is not None else " "
            
        mean_train_err = None  # keep last epoch's mean train error
        for epoch in range(1,num_epochs+1):
           # time_start = time()
            ...
            mean_train_err = torch.mean(train_errors) if self.__use_torch else np.mean(train_errors)
             # use a float for comparisons/logging
            mean_train_val = float(mean_train_err.detach().cpu().item()) if self.__use_torch else float(mean_train_err)
            if num_epochs > 1:
                lbl = _loss_short(cost_type)
                if cost_type in ("bicross_entropy", "cross_entropy"):
                    # show accuracy for classification
                    acc_tr = self.compute_accuracy(data, cost_type=cost_type, on="train")
                    logger.info(f"{tag}\tColony({self.colony_id:3d}){thd}:: "
            f"Epoch: {epoch:3d}/{num_epochs} Epoch {lbl}: {float(mean_train_val):.6e} | ACC: {acc_tr:.2f}%")
                else:
                    # keep MSE for regression
                    logger.info(f"{tag}\tColony({self.colony_id:3d}){thd}:: "
            f"Epoch: {epoch:3d}/{num_epochs} Epoch {lbl}: {float(mean_train_val):.6e}")

                if mean_train_val > 10:
                    break
        ...
        mean_train_val = float(mean_train_err.detach().cpu().item()) if (self.__use_torch and mean_train_err is not None) else (float(mean_train_err) if mean_train_err is not None else float("nan"))
        mean_test_val  = float(mean_test_err.detach().cpu().item())  if self.__use_torch else float(mean_test_err)
        lbl = _loss_short(cost_type)
        if cost_type in ("bicross_entropy", "cross_entropy"):
            acc_tr = self.compute_accuracy(data, cost_type=cost_type, on="train")
            acc_te = self.compute_accuracy(data, cost_type=cost_type, on="test")
            logger.info(f"Colony({self.colony_id}){thd}:: "
                        f"Train {lbl}: {float(mean_train_val):.6e}, "
                        f"Test {lbl}: {float(mean_test_val):.6e}, "
                        f"ACC(train/test): {acc_tr:.2f}%/{acc_te:.2f}%")
        else:
            logger.info(f"Colony({self.colony_id}){thd}:: "
                        f"Training MSE: {float(mean_train_val):.6e}, "
                        f"TEST MSE: {float(mean_test_val):.6e}")
