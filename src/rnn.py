...
class RNN:
    ...
    def do_epoch(
        ...
        for i in range(len(inputs) - self.lags):
            ...
            print("--->", err, len(err))
            err = sum(err) / len(err)
            self.total_err += err.item()
            if do_feedbck:
                self.feedbackward(err, lr, active_inference)
            for node in nodes.values():
                node.reset_node()

        self.total_err /= i
        logger.info(f"Colony({self.colony_id}):: Worker({self.worker}): Training Epoch average Total Error: {self.total_err}")
    ...
    def feedbackward(self, err, lr, active_inference=False) -> None:
        """
