...
class Point:
    ...
    def __init__(self, x, y, z, w, f, type, name="", index=None):
        self.__x = x
        self.__y = y
        self.__z = z
        self.__w = w
        ...
        Point.count += 1
        logger.debug(
            f"Creating Point({self.__id}): x={x}, y={y}, z={z}, f={f}, Type={type}, Name={name}"
        )
    ...
    def coordinates(self):
        return [self.__x, self.__y, self.__z, self.__w, self.__f]

    def distance_to(self, x, y, z, w, f):
        # logger.debug(f"Calculating distance from Point({self.__id}) to ({x}, {y}, {z} {w}, {f})")
        # logger.debug(f"Point doing the distance: ({self.__x}, {self.__y}, {self.__z}, {self.__w}, {self.__f})")
        return np.sqrt(
            (self.__x - x) ** 2
            + (self.__y - y) ** 2
            + (self.__z - z) ** 2
...
class Space:
    def __init__(
        ...
        for idx, output_name in enumerate(self.output_names):
            ...
            self.output_points[output_name] = Point(
                ...
                y=1.0,  # y=0 and x between 0-1 and z=0
                z=1.0,
                w=0.0,  # w=0 for output nodes
                f=0.0,
                type=2,
                name=output_name,
                index=idx,
    ...
    def evaporate_pheromone(self, evaporation_rate):
        ...
        for (
            point
        ) in self.input_points:  # Remove points with low pheromone (input nodes)
            if point.get_pheromone() < 0.1:
                points_to_remove.append(point)
        for point in points_to_remove:
